...
.detail-content img { max-width: 100%; border-radius: 8px; margin-bottom: 10px; }
    ...
    <?php if (isset($_SESSION['user_id'])): ?>
        <div class="cart" style="display:inline-block; margin-left:10px;">
            <a href="cart.php" style="color: white;">
                ...
                <span class="cartlength"><?= isset($_SESSION['cart']) ? count($_SESSION['cart']) : 0 ?></span>
            </a>
        </div>
    <?php endif; ?>
    <button id="show-hide" onclick="ShowHide()"><i class="fa-solid fa-bars"></i></button>
</header>
...
<div class="search-bar-container">
    <div class="textarea"><h3>Rent the equipment you need, when you need!</h3></div>
    <div class="textarea1"><h6>Smartly search the equipment and make a rent here.</h6></div>
    <form method="GET" class="searchbar-container">
        <input type="search" name="search" placeholder="Search for equipment..." value="<?= htmlspecialchars($searchQuery ?? '') ?>" />
        ...
        ?>
            <div class="card">
                ...
                <div class="button-container">
                    ...
                    <a href="index.php?action=add_to_cart&item_id=<?= $row['id'] ?>" class="cart-btn"><i class="fas fa-cart-plus"></i> Add to Cart</a>
                </div>
            </div>
        <?php endwhile; ?>
    </div>
...
<!-- Login Modal -->
<div id="login-modal" class="login-modal hidden"> ... </div>
<!-- Register Modal -->
<div id="register-modal" class="register-modal hidden"> ... </div>

<footer class="site-footer"> ... </footer>

<!-- JavaScript to handle equipment detail modal -->
<script>
document.addEventListener("DOMContentLoaded", function() {
    const modal = document.getElementById("equipment-detail");
    const closeBtn = document.getElementById("close-detail");
    const detailImg = document.getElementById("detail-image");
    const detailName = document.getElementById("detail-name");
    const detailAvailability = document.getElementById("detail-availability");
    const detailDescription = document.getElementById("detail-description");
    const rentBtn = document.getElementById("detail-rent");
    const cartBtn = document.getElementById("detail-cart");

    document.querySelectorAll(".equipment-img").forEach(img => {
        img.addEventListener("click", () => {
            detailImg.src = img.dataset.image;
            detailName.textContent = img.dataset.name;
            detailAvailability.textContent = "Availability: " + img.dataset.availability;
            detailDescription.textContent = img.dataset.description;

            rentBtn.onclick = () => {
                window.location.href = `index.php?action=rent_now&item_id=${img.dataset.id}`;
            };
            cartBtn.onclick = () => {
                window.location.href = `index.php?action=add_to_cart&item_id=${img.dataset.id}`;
            };
